...
const AllAppointment = () => {
  ...
  const filteredAppointments = useMemo(() => {
    return appointments
      ...
      .filter(item => {
        ...
        return item.docId?._id === selectedDoctor;
      })
      .filter(item => {
      ...
      .sort((a, b) => {
        ...
        const priority = (x) => x.cancelled ? 1 : 0; // cancelled = lower priority

        if (priority(a) !== priority(b)) return priority(a) - priority(b);
        return sortOrder === "asc" ? dateA - dateB : dateB - dateA;
      });
  }, [appointments, filter, paymentFilter, selectedDoctor, dateRange, search, sortOrder]);
  ...
  <div className="m-5 max-h-[90vh] overflow-y-auto scroll-smooth">
    ...
    <div className="flex flex-wrap items-center gap-2 mb-4">
      ...
      <select onChange={(e) => setSortOrder(e.target.value)} value={sortOrder} className="border border-gray-300 px-3 py-2 rounded text-sm">
        <option value="asc">Oldest First</option>
        <option value="desc">Newest First</option>
      </select>

      <select onChange={(e) => setSelectedDoctor(e.target.value)} value={selectedDoctor} className="border border-gray-300 px-3 py-2 rounded text-sm">
        <option value="all">All Doctors</option>
        {[...new Set(appointments.map((a) => a.docId?._id))].map((id) => {
          const doc = appointments.find((a) => a.docId?._id === id)?.docId;
          return <option key={id} value={id}>{doc?.name || "Unknown"}</option>;
        })}
      </select>

      <input type="date" value={dateRange.from} onChange={(e) => setDateRange({ ...dateRange, from: e.target.value })} className="border px-3 py-2 rounded text-sm" />
      <input type="date" value={dateRange.to} onChange={(e) => setDateRange({ ...dateRange, to: e.target.value })} className="border px-3 py-2 rounded text-sm" />
      ...
      <div className="min-w-[1100px] space-y-1">
        ...
        {paginatedAppointments.length > 0 ? (
          paginatedAppointments.map((item, index) => (
            <div key={item._id} className={`grid grid-cols-12 gap-2 items-center text-sm border-b border-gray-200 py-2 ${item.cancelled ? "bg-red-50" : "hover:bg-blue-50"} transition rounded`}>
              <p className="col-span-1">{(currentPage - 1) * 10 + index + 1}</p>
